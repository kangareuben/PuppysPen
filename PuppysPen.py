...
class PuppysPen:
    ...
    def run(self):
        ...
        self.play_screen = GameScreen(self.py_screen, self)
...
class GameScreen(Screen):
    def __init__(self, _py_screen, _game):
        ...
        self.game = _game
        self.py_screen = _py_screen
        self.grid_offset = self.game.center_coords(600, 400)
        self.mouse_grid_position = (0, 0)
        self.mouse_prev_grid_position = (0, 0)
    ...
    def update(self):
        self.instruction_text_surface, self.instruction_text_pos = self.game.draw_text("Click once to start a rectangle, and again to finish it")
        self.instruction_text_pos.centerx = self.py_screen.get_rect().centerx
    ...
    def mousemove(self, pos):
        ...
        if (mouse_grid_x, mouse_grid_y) != self.mouse_prev_grid_position:
            ...
            if not self.drawing_rect:
                ...
                origin_x = int(self.grid_offset[0] + (self.game.rect_origin[0] * self.column_width))
                origin_y = int(self.grid_offset[1] + (self.game.rect_origin[1] * self.row_height))
